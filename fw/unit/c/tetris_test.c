...
const uint8_t TETRIS_BLOCK_DISPLAY[] = {0x20, 0xB0, 0xB1, 0xDB}; 
...
// no grab bag for now
void get_next_piece() {
  tetris_curr_idx = rand() % 7;
  tetris_curr_x = 4;
  tetris_curr_row = TETRIS_WELL_USABLE_DEPTH;
}
...
int oob(int x, int y) {
  return (x < 0 || x >= TETRIS_WELL_WIDTH || y < 0 || y >= TETRIS_WELL_USABLE_DEPTH);
}
...
void tetris_display_well(int x, int y) {
  ...
  while (j < TETRIS_WELL_USABLE_DEPTH) {
    while (i < TETRIS_WELL_WIDTH) {
      ...
      char color = has_cell(i,j) ? TETRIS_BLOCK_COLORS[(tetris_well[j][i] & TETRIS_BLOCK_COLOR_MASK) >> 4] : 0x00;
      vga_print_char(
          x + i, 
...
void tetris_move_piece_down() {
  // if we get a collision,
  if (tetris_collides_in_well(0,-1,0)) {
...
void tetris_clear_line(int row) {
  ...
  while(max_frames > curr_frame) {
    ...
    while (i < TETRIS_WELL_WIDTH) {
      ...
      tetris_well[row][l_offset] = cell_data ? ((tetris_well[row][l_offset] & (TETRIS_BLOCK_COLOR_MASK) >> 4) + cell_data : 0;
      tetris_well[row][r_offset] = cell_data ? ((tetris_well[row][r_offset] & (TETRIS_BLOCK_COLOR_MASK) >> 4) + cell_data : 0;
      i++;
    }
...
void tetris_game_over() {
  vga_draw_box(36,9,43,12,0x0f,0x0f);
  vga_print_plain(37,10,"GAME");
  vga_print_plain(38,11,"OVER!");
}
...
void tetris_window() {
  ...
  vga_draw_32bit_debug_str(0,1,tetris_curr_idx);
  vga_draw_32bit_debug_str(0,2,tetris_curr_x);
  vga_draw_32bit_debug_str(0,3,tetris_curr_row);
  vga_draw_32bit_debug_str(0,4,tetris_curr_rot);

  vga_print_plain(0,6,"-LAST TIME-");
  vga_draw_32bit_debug_str(0,7,last_time);
  vga_draw_32bit_debug_str(0,8,tetris_curr_g);
  
  // draw line count
  vga_draw_box(34,1,45,3,0x0f,0x0f);
  //write_integer(tetris_lines, tetris_line_buf);
  vga_print_plain(35,2,"LINES - ");
  vga_print_plain_buf(43,2,tetris_line_buf,2);
  
  // draw well
  vga_draw_box(34,5,45,27,0x0f,0x0f);
  ...
  piece_traversal(FN_TETRIS_VGA_DRAW, tetris_curr_idx, 0, 48, 11);
  
  // draw current piece
  piece_traversal(FN_TETRIS_VGA_DRAW, tetris_curr_idx, tetris_curr_rot, tetris_curr_x + TETRIS_WELL_TOP_X, TETRIS_WELL_TOP_Y + (20 - tetris_curr_row)); 
}
...
void tetris(char c) {
  ...
  if (time != last_time) {
    ...
    // check for a line clear on each row, top to bottom
    // animation will play in series for each row cleared
    ...
    // is there a tetromino on the top rows?
    if (tetris_has_in_row(21)) {
      tetris_game_over();
    }
...
int main() {
  ...
  while(1) {
    ...
    char c = !(input & MASK_MAKE) ? (char)(input & 0x000000ff) : 0;
    int status = !(input & MASK_MAKE) ? (input & 0xffffff00) >> 8 : 0;
