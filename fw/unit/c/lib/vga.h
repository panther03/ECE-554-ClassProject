...
void vga_draw_hex_debug_str(int x, int y, int data) {
  volatile uint16_t* buff_addr = (volatile uint16_t*)VGA_TEXT_BUFFER + (y << 4) + (y << 6) + x;
  int i = 0;
  while (i < 8) {
    int byte = (data & (0x0000000f << (i << 2))) >> (i << 2);
    unsigned short hex_char = byte < 10 ? 0x0030 + byte : 0x0041 + (byte - 10);
    *(buff_addr) = byte ? (0x2a00 | hex_char) : (0x4c30);
    buff_addr++;
    i++;
  }
}
